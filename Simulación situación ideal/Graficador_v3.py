"""
Graficador v3 — Estrategias BTC/USDT
══════════════════════════════════════════════════════════════════
...
Ejecutar:
    python Graficador_v3.py
    python Graficador_v3.py --json resultados.json --db datos.db --dark
"""
...
class GraficadorV3:
    ...
    def crear_grafico(self, output=OUTPUT_PNG):
        ...
        # Score panel eliminado en v3 — siempre 6 paneles
        # ── Layout ────────────────────────────────────────────────
        n_panels      = 6
        height_ratios = [3.2, 1.1, 0.9, 0.75, 1.5, 0.95]

        fig = plt.figure(figsize=(22, 24), facecolor=C["bg"])
        gs  = gridspec.GridSpec(n_panels, 1, figure=fig,
                                height_ratios=height_ratios,
                                hspace=0.12, top=0.960, bottom=0.035,
        ...
        ax.text(0.99, 0.97, box_txt, transform=ax.transAxes, fontsize=7.5,
        ...
        ax.set_title("Precio BTC + Operaciones", color=C["text"], fontsize=10, pad=6)
        ...
        ax.annotate(f"${usdt_fin:,.2f}",
                    ...
                    arrowprops=dict(arrowstyle="->", color=C["usdt_line"], alpha=0.6, lw=0.8))
        ax.set_ylabel("Balance USDT", color=C["text"], fontsize=9)
        ax.set_title("Evolución del Balance USDT", color=C["text"], fontsize=10, pad=6)
        ...
        ax.set_title("Evolución del Balance BTC", color=C["text"], fontsize=10, pad=6)
        ...
        ax.set_title("Posiciones Abiertas Netas", color=C["text"], fontsize=10, pad=6)
        ax.xaxis.set_visible(False)
...
def main():
    parser = argparse.ArgumentParser(description="Graficador v3 — Estrategias BTC")
    ...
    print("╔══════════════════════════════════════════════════════════╗")
    print("║        GRAFICADOR v3 — ESTRATEGIAS BTC/USDT             ║")
    print("╚══════════════════════════════════════════════════════════╝\n")

    g = GraficadorV3(dark_mode=dark)
