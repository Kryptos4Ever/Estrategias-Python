"""
Estrategia Divergencia RSI — Umbral de Zona · Gradiente Asintótico
══════════════════════════════════════════════════════════════════════
...
COMPRA (divergencia alcista con filtro de sobreventa):
  ...
  · rsi_low[idx_min] ≤ RSI_BUY_TRIGGER          ← âncla en sobreventa
  · low[i]       < window_lows.min()             → nuevo mínimo local
  · RSI(low[i])  > RSI(low[idx_min])             → RSI no confirma
...
VENTA (divergencia bajista con filtro de sobrecompra):
  ...
  · rsi_high[idx_max] ≥ RSI_SELL_TRIGGER         ← âncla en sobrecompra
  · high[i]      > window_highs.max()            → nuevo máximo local
  · RSI(high[i]) < RSI(high[idx_max])            → RSI no confirma

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
GRADIENTE ASINTÓTICO DE COMPRA
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ATL_REF   = ATH × FLOOR_PCT / 100       ← se recalcula con cada nuevo ATH
  log_rango = log(100 / FLOOR_PCT)        ← amplitud logarítmica del ciclo
  pos       = log(ATH / precio) / log_rango   ∈ [0, 1]

  Función de dos fases sobre pos:
    Fase 1  [0, COMPRA_INFL]:
      pct = COMPRA_NIVEL × (1 − e^(−k·pos)) / (1 − e^(−k·COMPRA_INFL))
      → Satura en COMPRA_NIVEL%. Cosecha temprana acotada.

    Fase 2  [COMPRA_INFL, 1]:
      pct = COMPRA_NIVEL + (100 − COMPRA_NIVEL) × ((pos − INFL)/(1 − INFL))^FAC2
      → Aceleración power hacia el 100% en ATL_REF.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
GRADIENTE ASINTÓTICO DE VENTA — ANCLADO AL PP con ATH_TEO
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ATH_TEO   = ATL / (FLOOR_PCT / 100)     ← se recalcula con cada nuevo ATL
  log_amp   = log(ATH_TEO / PP)
  pos       = log(precio / PP) / log_amp  ∈ [0, 1]

  Misma función de dos fases sobre pos:
    Fase 1  [0, VENTA_INFL]:
      pct = VENTA_NIVEL × (1 − e^(−k·pos)) / (1 − e^(−k·VENTA_INFL))
      → Cosecha temprana hasta VENTA_NIVEL% del BTC.

    Fase 2  [VENTA_INFL, 1]:
      pct = VENTA_NIVEL + (100 − VENTA_NIVEL) × ((pos − INFL)/(1 − INFL))^FAC2
      → Distribución agresiva hacia ATH_TEO.

  La guardia de venta está incorporada: pos ≤ 0 cuando precio ≤ PP → pct = 0.

  Diferencia clave vs versión anterior:
    Antes : techo de venta = ATH real (puede ser inalcanzable o estar muy lejos)
    Ahora : techo de venta = ATH_TEO = ATL / FLOOR_PCT
            → El rango se calibra simétricamente al ciclo observado.
            → ATH_TEO se actualiza con cada nuevo mínimo del mercado,
              achicando el techo teórico si el bear se profundiza.
"""
...
from config import (
    ...
    FLOOR_PCT,
    TOP_PCT,
    CURVA_COMPRA_NIVEL, CURVA_COMPRA_INFL, CURVA_COMPRA_K, CURVA_COMPRA_FAC2,
    CURVA_VENTA_NIVEL,  CURVA_VENTA_INFL,  CURVA_VENTA_K,  CURVA_VENTA_FAC2,
    GUARDIA_COMPRA,
...
def _gradiente_asintotico(pos: float,
                           nivel: float, infl: float,
                           k: float, fac2: float) -> float:
    """
    Función de dos fases [0, 100] sobre pos ∈ [0, 1].

    Fase 1  [0, infl]:
        Exponencial que satura en 'nivel'%.
        scale = 1 − e^(−k·infl)  asegura que en pos=infl el output = nivel.

    Fase 2  [infl, 1]:
        Power que acelera desde nivel% hasta 100% en pos=1.

    Retorna 0 para pos ≤ 0.
    """
    if pos <= 0.0:
        return 0.0
    if pos <= infl:
        scale = 1.0 - math.exp(-k * infl)
        if scale <= 0:
            return 0.0
        return nivel * (1.0 - math.exp(-k * pos)) / scale
    else:
        x2 = (pos - infl) / (1.0 - infl)
        return nivel + (100.0 - nivel) * (x2 ** fac2)
...
def pct_capital_compra(precio_low: float, ath: float) -> float:
    ...
    Dominio de precios:
      pos = 0  cuando precio = ATH      → no compra
      pos = 1  cuando precio = ATL_REF  → 100% del capital disponible

    ATL_REF = ATH × FLOOR_PCT / 100  (constante para cada ATH registrado).
    """
    ...
    return _gradiente_asintotico(pos,
                                  CURVA_COMPRA_NIVEL, CURVA_COMPRA_INFL,
                                  CURVA_COMPRA_K,     CURVA_COMPRA_FAC2)


def pct_capital_venta(precio_high: float, ath_teo: float,
                       precio_promedio: float) -> float:
    """
    Gradiente asintótico de venta anclado al PP y acotado por ATH_TEO.

    Dominio de precios:
      pos = 0  cuando precio = PP       → no vende (guardia incorporada)
      pos = 1  cuando precio = ATH_TEO  → 100% del BTC en posiciones

    ATH_TEO = ATL_dinámico × TOP_PCT / 100 — se recibe como argumento
    porque se actualiza en tiempo real en el loop principal.

    Retorna 0 si precio ≤ PP o si ATH_TEO ≤ PP.
    """
    if ath_teo <= 0 or precio_promedio <= 0:
        return 0.0
    if precio_high <= precio_promedio:
        return 0.0                          # guardia incorporada
    if ath_teo <= precio_promedio:
        return 0.0                          # rango degenerado
    log_amp = math.log(ath_teo / precio_promedio)
    if log_amp <= 0:
        return 0.0
    pos = math.log(precio_high / precio_promedio) / log_amp
    pos = max(0.0, min(1.0, pos))
    return _gradiente_asintotico(pos,
                                  CURVA_VENTA_NIVEL, CURVA_VENTA_INFL,
                                  CURVA_VENTA_K,     CURVA_VENTA_FAC2)
...
def _registro_ignorado(ts, tipo, precio, rsi_l, rsi_h,
                        ...
                        pos_count, ath, atl, ath_teo, motivo: str) -> dict:
    return {
...
def ejecutar_estrategia(df: pd.DataFrame) -> dict:
    ...
    # ── ATH / ATL — inicializar con las primeras N velas ──────────────────────
    # El loop empieza en i=N, así que highs/lows[1..N-1] quedarían invisibles
    # si solo usáramos el índice 0.
    ath = float(np.max(highs[:N]))
    atl = float(np.min(lows[:N]))

    # ATH_TEO: techo teórico de venta = ATL × TOP_PCT / 100
    # Se actualiza siempre que ATL baja (nuevo mínimo histórico observado).
    # TOP_PCT es independiente de FLOOR_PCT, permitiendo calibrar cada lado
    # del ciclo por separado:
    #   ATL_REF = ATH × FLOOR_PCT/100   →  piso esperado del bear market
    #   ATH_TEO = ATL × TOP_PCT/100     →  techo esperado del bull market
    ath_teo = atl * TOP_PCT / 100.0
    ...
    for i in range(N, n_velas):
        ...
        # ── Actualizar ATH / ATL / ATH_TEO ───────────────────────────────────
        if highs[i] > ath:
            ath = float(highs[i])
        if lows[i] < atl:
            atl     = float(lows[i])
            ath_teo = atl * TOP_PCT / 100.0   # techo teórico se recalibra
            ...
            if rsi_low[i] > rsi_low[idx_min]:
                ...
                if rsi_low[idx_min] <= RSI_BUY_TRIGGER:
                    ...
                    trade_history.append(_registro_ignorado(
                        ...
                        positions_count, ath, atl, ath_teo,
        ...
        if señal_compra:
            ...
            if usdt_disponible <= 0:
                trade_history.append(_registro_ignorado(
                    ts, "BUY", lows[i], rsi_low[i], rsi_high[i],
                    usdt_balance, btc_balance, btc_en_posiciones,
                    positions_count, ath, atl, ath_teo, "sin_capital_sobre_reserva"
                ))
                continue
            ...
            if GUARDIA_COMPRA and btc_en_posiciones > 0 and lows[i] >= precio_promedio:
                trade_history.append(_registro_ignorado(
                    ts, "BUY", lows[i], rsi_low[i], rsi_high[i],
                    usdt_balance, btc_balance, btc_en_posiciones,
                    positions_count, ath, atl, ath_teo, "precio_sobre_promedio"
                ))
                continue

            if GUARDIA_PRECIO_COMPRA and precio_min_comprado < math.inf \
                    and lows[i] >= precio_min_comprado:
                trade_history.append(_registro_ignorado(
                    ts, "BUY", lows[i], rsi_low[i], rsi_high[i],
                    usdt_balance, btc_balance, btc_en_posiciones,
                    positions_count, ath, atl, ath_teo, "precio_sobre_min_comprado"
                ))
                continue
            ...
            if usdt_a_usar <= 0:
                trade_history.append(_registro_ignorado(
                    ts, "BUY", lows[i], rsi_low[i], rsi_high[i],
                    usdt_balance, btc_balance, btc_en_posiciones,
                    positions_count, ath, atl, ath_teo, "gradiente_cero"
                ))
                continue

            # pos_gradiente: posición normalizada en el rango de compra
            log_rango = math.log(100.0 / FLOOR_PCT)
            pos_grad  = round(math.log(ath / lows[i]) / log_rango, 6) \
                        if log_rango > 0 else 0.0
        ...
        # ─────────────────────────────────────────────────────────────────────
            ...
            # pct_capital_venta recibe ath_teo (dinámico) en lugar del ATH real
            pct      = pct_capital_venta(highs[i], ath_teo, precio_promedio)
            ...
            if btc_slot <= 0:
                ...
                trade_history.append(_registro_ignorado(
                    ts, "SELL", highs[i], rsi_low[i], rsi_high[i],
                    usdt_balance, btc_balance, btc_en_posiciones,
                    positions_count, ath, atl, ath_teo, motivo
                ))
                continue

            if GUARDIA_PRECIO_VENTA and precio_max_vendido > 0 \
                    and highs[i] <= precio_max_vendido:
                trade_history.append(_registro_ignorado(
                    ts, "SELL", highs[i], rsi_low[i], rsi_high[i],
                    usdt_balance, btc_balance, btc_en_posiciones,
                    positions_count, ath, atl, ath_teo, "precio_bajo_max_vendido"
                ))
                continue

            # pos_gradiente: posición normalizada en el rango de venta
            log_amp  = math.log(ath_teo / precio_promedio) \
                       if ath_teo > precio_promedio > 0 else 0.0
    ...
    summary = {
        ...
        "ath_teo_final"           : round(ath_teo, 4),   # ATL × TOP_PCT/100
        ...
        "parametros": {
            ...
            "curva_compra_nivel"   : CURVA_COMPRA_NIVEL,
            "curva_compra_infl"    : CURVA_COMPRA_INFL,
            "curva_compra_k"       : CURVA_COMPRA_K,
            "curva_compra_fac2"    : CURVA_COMPRA_FAC2,
            # Curva asintótica de venta
            "curva_venta_nivel"    : CURVA_VENTA_NIVEL,
            "curva_venta_infl"     : CURVA_VENTA_INFL,
            "curva_venta_k"        : CURVA_VENTA_K,
            "curva_venta_fac2"     : CURVA_VENTA_FAC2,
            # Resto
...
def main():
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║  DIVERGENCIA RSI — UMBRAL · GRADIENTE ASINTÓTICO            ║")
    ...
    print(f"  ATH_TEO final        : ${s['ath_teo_final']:>10,.2f}   (= ATL × {TOP_PCT/100:.2f}  |  TOP_PCT={TOP_PCT}%)")
