"""
Optimizador Completo — Divergencia RSI Umbral  [VERSIÓN VECTORIZADA]
════════════════════════════════════════════════════════════════════
...
────────────────────────────────────────────────────────────────
1. Precomputo de señales por (RSI_LENGTH, N)
   Los eventos de divergencia se detectan una sola vez por par
   (RSI_LENGTH, N). El backtest opera sobre ~300-700 señales en
   lugar de 9,049 velas → reducción 10-30x por iteración.

2. Simulación vectorizada con NumPy
   Para cada señal, el estado de TODAS las combinaciones se
   actualiza simultáneamente con operaciones numpy.
   Sin Numba, sin threads, sin procesos. Solo numpy puro.

3. Pre-cómputo de RSI con caché
   El RSI solo depende de RSI_LENGTH → se calcula una vez por
   valor, no por combinación.

Resultado esperado: ~30-90 segundos para 1,866,240 combinaciones.
Sin dependencias adicionales: solo numpy, pandas, matplotlib.
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# ESPACIO DE BÚSQUEDA  (idéntico al original)
# ══════════════════════════════════════════════════════════════════════════════

RSI_LENGTH_INICIO    = 6  ;  RSI_LENGTH_FIN    = 24  ;  RSI_LENGTH_PASO    = 3
N_INICIO             = 12 ;  N_FIN             = 48  ;  N_PASO             = 6
FLOOR_PCT_INICIO     = 10 ;  FLOOR_PCT_FIN     = 25  ;  FLOOR_PCT_PASO     = 5
FACTOR_CAIDA_INICIO  = 0.5;  FACTOR_CAIDA_FIN  = 5.0 ;  FACTOR_CAIDA_PASO  = 0.5
FACTOR_SUBIDA_INICIO = 0.5;  FACTOR_SUBIDA_FIN = 4.0 ;  FACTOR_SUBIDA_PASO = 0.5
...
#   precio: low (compra) o high (venta)
#   ath   : ATH vigente en ese momento
#   anc   : RSI del ancla (mínimo o máximo previo de la ventana)
#   close : close de esa vela (para mark-to-market)
# ══════════════════════════════════════════════════════════════════════════════
...
def precompute_signals(lows, highs, closes, rsi_low, rsi_high, N):
    ...
    sig_anc   = []
    sig_close = []
    # Inicializar ATH con el máximo de las primeras N velas.
    # El loop empieza en i=N — highs[1..N-1] quedarían invisibles
...
# ══════════════════════════════════════════════════════════════════════════════
# Recibe las señales precomputadas y el grid de M combinaciones.
# Para cada señal, actualiza el estado de TODAS las M combinaciones en paralelo
# mediante operaciones numpy. No hay loop Python sobre combinaciones.
# ══════════════════════════════════════════════════════════════════════════════
...
def simular_vectorizado(signals, combos_arr, usdt_ini, usdt_res, comm_pct, btc_acum):
    ...
    fc_arr  = combos_arr[:, 1]                    # factor_caida
    fs_arr  = combos_arr[:, 2]                    # factor_subida
    bt_arr  = combos_arr[:, 3]                    # rsi_buy_trigger
    st_arr  = combos_arr[:, 4]                    # rsi_sell_trigger
    gc_arr  = combos_arr[:, 5].astype(bool)       # guardia_compra
    gpc_arr = combos_arr[:, 6].astype(bool)       # guardia_prec_compra
    gpv_arr = combos_arr[:, 7].astype(bool)       # guardia_prec_venta

    log_r_arr = np.log(100.0 / fp_arr)
    comm      = comm_pct / 100.0
    ...
    for j in range(S):
        ...
        if stype[j] == 0:   # ════ COMPRA ════
            ...
            trigger_mask = anc <= bt_arr          # solo filtro de trigger
            div_c_apr   += trigger_mask.astype(np.int32)  # aprobación pura de trigger
            ...
            pos = np.clip(log_ratio / log_r_arr, 0.0, 1.0)
            ua  = usdt * (pos ** fc_arr)
            mask &= ua > 1e-8
            ...
            # can_sell: tiene posición, precio > PP, ATH > PP (log válido)
            can_sell     = has_pos & (pp > 0.0) & (price > pp) & (ath_j > pp)
            trigger_mask = can_sell & (anc >= st_arr)   # filtro de trigger puro
            div_v_apr   += trigger_mask.astype(np.int32)

            mask = trigger_mask.copy()

            safe_pp  = np.where(can_sell, pp, ath_j * 0.99)
            log_a    = np.where(can_sell, np.log(ath_j / safe_pp), 1.0)
            log_a    = np.maximum(log_a, 1e-10)
            log_num  = np.where(can_sell,
                                np.log(np.maximum(price / safe_pp, 1.0001)), 0.0)
            pos      = np.where(can_sell, np.clip(log_num / log_a, 0.0, 1.0), 0.0)
            slot     = btc_pos * (pos ** fs_arr)
            mask    &= slot > 1e-12
    ...
    # Retornar dict de arrays — el llamador extrae solo los índices top-K
    return {
        "pnl_pct"        : pnl_pct,
        "portfolio"      : portfolio,
...
# Cuántos mejores resultados conservar por cada métrica de ranking.
# La unión de los tres heaps → máx ~TOP_HEAP*3 filas en disco (≈ 300 KB).
TOP_HEAP = 500
...
# ══════════════════════════════════════════════════════════════════════════════
# Mantiene los TOP_HEAP mejores resultados por una métrica.
# Cuando llega un valor nuevo:
#   · Heap no lleno                  → entra siempre
#   · Valor > mínimo actual del heap → entra, expulsa al peor
#   · Valor ≤ mínimo actual          → se descarta en O(1)
# ══════════════════════════════════════════════════════════════════════════════
...
def optimizar(df: pd.DataFrame) -> pd.DataFrame:
    ...
    En lugar de volcar todos los resultados a disco (→ 6 GB), mantiene
    en RAM tres heaps de 500 entradas cada uno:
      · heap_pnl : mejores por pnl_pct
      · heap_btc : mejores por btc_posiciones  (proxy de btc_value)
      · heap_eq  : mejores por sqrt(pnl_pos × btc) (proxy de equilibrio)

    Un resultado entra a un heap solo si supera al peor actualmente
    almacenado en esa métrica. La unión deduplicada al final da ≤1500
    filas únicas (~300 KB en CSV vs 6 GB del enfoque anterior).

    Los scores finales (btc_value, equilibrio_score) se calculan sobre
    ese conjunto reducido con precio_final real.
    """
    ...
    print(f"  FLOOR_PCT        : {floor_pcts[0]} → {floor_pcts[-1]}  ({len(floor_pcts)} vals)")
    print(f"  FACTOR_CAIDA     : {factor_caidas[0]} → {factor_caidas[-1]}  ({len(factor_caidas)} vals)")
    print(f"  FACTOR_SUBIDA    : {factor_subidas[0]} → {factor_subidas[-1]}  ({len(factor_subidas)} vals)")
    ...
    # Construir array de parámetros sin RSI_LENGTH y N — shape (M, 8)
    param_combos = list(product(
        floor_pcts, factor_caidas, factor_subidas,
        buy_triggers, sell_triggers,
        guardias_bool, guardias_bool, guardias_bool,
    ))
    combos_arr = np.array(
        [(fp, fc, fs, float(bt), float(st),
          1.0 if gc else 0.0,
          1.0 if gpc else 0.0,
          1.0 if gpv else 0.0)
         for (fp, fc, fs, bt, st, gc, gpc, gpv) in param_combos],
        dtype=np.float64
    )
    M = len(combos_arr)

    # Tres heaps — uno por métrica de ranking
    heap_pnl = _TopHeap(TOP_HEAP)   # mejor pnl_pct
    heap_btc = _TopHeap(TOP_HEAP)   # mejor btc_posiciones
    heap_eq  = _TopHeap(TOP_HEAP)   # mejor sqrt(pnl_pos × btc)

    print(f"  Modo    : vectorizado NumPy + heaps top-{TOP_HEAP}")
    print(f"  Disco   : solo los mejores ~{TOP_HEAP*3:,} resultados (no CSV parcial de 6 GB)\n")
    ...
    for rsi_len in rsi_lengths:
        ...
        for n_val in ns:
            ...
            if signals is not None:
                ...
                for i in cand_idx:
                    fp, fc, fs, bt, st, gc, gpc, gpv = param_combos[i]
                    row = {
                        "rsi_length"         : rsi_len,
                        "N"                  : n_val,
                        "floor_pct"          : fp,
                        "factor_caida"       : fc,
                        "factor_subida"      : fs,
                        "rsi_buy_trigger"    : float(bt),
                        "rsi_sell_trigger"   : float(st),
                        "guardia_compra"     : gc,
                        "guardia_prec_compra": gpc,
                        "guardia_prec_venta" : gpv,
                        "pnl_pct"            : round(float(arrs["pnl_pct"][i]),          4),
                        "portfolio_final"    : round(float(arrs["portfolio"][i]),         2),
                        "usdt_final"         : round(float(arrs["usdt_final"][i]),        2),
                        "btc_posiciones"     : round(float(arrs["btc_posiciones"][i]),    8),
                        "precio_prom_fin"    : round(float(arrs["precio_prom_fin"][i]),   2),
                        "total_trades"       : int(arrs["total_trades"][i]),
                        "total_compras"      : int(arrs["compras"][i]),
                        "total_ventas"       : int(arrs["ventas"][i]),
                        "positions_count"    : int(arrs["pos_count"][i]),
                        "pnl_por_trade"      : round(float(arrs["pnl_por_trade"][i]),    4),
                        "max_drawdown"       : round(float(arrs["max_dd"][i]),            2),
                        "div_compra_det"     : int(arrs["div_c_det"][i]),
                        "div_compra_apr"     : int(arrs["div_c_apr"][i]),
                        "tasa_apr_compra"    : round(float(arrs["tasa_c"][i]),            1),
                        "div_venta_det"      : int(arrs["div_v_det"][i]),
                        "div_venta_apr"      : int(arrs["div_v_apr"][i]),
                        "tasa_apr_venta"     : round(float(arrs["tasa_v"][i]),            1),
                    }
                    pv = float(pnl_arr[i])
                    bv = float(btc_arr[i])
                    ev = float(eq_prx[i])
                    heap_pnl.push_if_better(pv, row)
                    heap_btc.push_if_better(bv, row)
                    heap_eq.push_if_better(ev, row)
    ...
    for heap in (heap_pnl, heap_btc, heap_eq):
        for row in heap.to_list():
            key = (row["rsi_length"], row["N"], row["floor_pct"],
                   row["factor_caida"], row["factor_subida"],
...
# ══════════════════════════════════════════════════════════════════════════════
# GUARDAR CSV + JSON  (idéntico al original)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# VISUALIZACIONES  (idénticas al original)
# ══════════════════════════════════════════════════════════════════════════════

_COLS = ["#", "RSI", "N", "BUY_T", "SELL_T", "FLOOR%", "F_CAI", "F_SUB",
         "G_PP", "G_PC", "G_PV",
...
def _fig_tabla(df_res, sort_col, highlight_col, cmap, titulo, subtitulo, filename):
    ...
    for rank, (_, r) in enumerate(top.iterrows(), 1):
        rows.append([
            ...
            f"{r.floor_pct:.0f}%", f"{r.factor_caida:.1f}", f"{r.factor_subida:.1f}",
    ...
    table.auto_set_font_size(False); table.set_fontsize(7.5); table.scale(1, 1.52)
    for j in range(len(_COLS)):
        c = table[0, j]
        c.set_facecolor("#1a2540"); c.set_text_props(color="white", fontweight="bold")
    for j in [3, 4]: table[0, j].set_facecolor("#1a4a80")
    table[0, highlight_col].set_facecolor("#e67e22")
    for j in [16, 17]: table[0, j].set_facecolor("#1e6b3c")
    ...
    ax.set_title(
        ...
        f"G_PP=Guardia precio prom.  ·  G_PC=Guardia precio mín. compra  ·  G_PV=Guardia precio máx. venta  |  "
...
def fig_analisis_guardias(df_res):
    ...
    fig = plt.figure(figsize=(28, 17)); fig.patch.set_facecolor("#f4f6fa")
    ...
    table.auto_set_font_size(False); table.set_fontsize(7.5); table.scale(1, 1.7)
    ...
    n_g_actual = len(df_g)   # puede ser < 8 si no todas las combos llegaron al top
...
def fig_analisis_variables(df_res):
    ...
    fig = plt.figure(figsize=(22, 25)); fig.patch.set_facecolor("#f4f6fa")
...
# ══════════════════════════════════════════════════════════════════════════════
# RESUMEN EN CONSOLA  (idéntico al original)
# ══════════════════════════════════════════════════════════════════════════════
...
def imprimir_resumen(df_res):
    ...
    hdr = (f"  {'#':>3}  {'RSI':>4}  {'N':>3}  {'BT':>4}  {'ST':>4}  "
           f"{'FL%':>4}  {'FC':>5}  {'FS':>5}  {'G_PP':>4}  {'G_PC':>4}  {'G_PV':>4}  "
    ...
    for titulo, ranked in rankings:
        ...
        print(f"\n  Dominantes — {titulo}")
        for col, lbl_ in [("rsi_length",      "RSI"),
                           ("N",               "N"),
...
def main():
    print("╔══════════════════════════════════════════════════════════════════════╗")
    print("║  OPTIMIZADOR — DIVERGENCIA RSI UMBRAL                                ║")
    print("║  NumPy Vectorizado  ×  Señales Precomputadas                         ║")
    print("╚══════════════════════════════════════════════════════════════════════╝\n")
